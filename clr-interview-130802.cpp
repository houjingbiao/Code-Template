...
所以，假设第j组含有毒酒，此时我们得到的没毒的就的瓶数就是n - n(j)。这样问题就可以转化为最优化问题
...
求解可以得到：
对n = 11，
当m = 1, 一次至少能鉴别n - min( max ( n(j) )) = 11 - 6 = 5;
当m = 2, 一次至少能鉴别n - min( max ( n(j) )) = 11 - 3 = 8;
当m = 3, 一次至少能鉴别n - min( max ( n(j) )) = 11 - 2 = 9;
当m = 4, 一次至少能鉴别n - min( max ( n(j) )) = 11 - 1 = 10;
